...
function calcLayer5(s) {
  ...
  // BLOCKER 2: Stock falling hard with high volume = distribution
  if ((s.changePct || 0) < -8) {
    blockers.push(`❌ Stock down ${Math.abs(s.changePct).toFixed(1)}% today — possible circuit event or bad news. Wait for next session.`);
  }
  if ((s.changePct || 0) < -3 && ind.volumeSignal === 'high') {
    blockers.push(`❌ Heavy selling (${s.changePct}%) with volume spike — distribution pattern. Avoid today.`);
  }

  // BLOCKER 3: Confirmed downtrend (below both MAs while falling)
  if (ind.sma50 && ind.sma20 && s.price && s.price < ind.sma20 && s.price < ind.sma50 && (s.changePct || 0) < -1) {
    blockers.push(`❌ Price below SMA20 (₹${ind.sma20}) and SMA50 (₹${ind.sma50}) while falling — confirmed downtrend. Skip this stock.`);
  }

  // SOFT WARNINGS (don't block but flag)
  if (dow === 4) {
    softWarns.push(`⚠️ Thursday F&O Expiry Day — high intraday volatility. Use 50% reduced position size.`);
  }
  if (timeIST > 9*60+15 && timeIST < 10*60+30) {
    softWarns.push(`⚠️ Before 10:30 AM IST — morning volatility window. Best to wait until 10:30 AM.`);
  }
  if (timeIST > 14*60+30 && timeIST < 15*60+30) {
    softWarns.push(`⚠️ After 2:30 PM IST — end-of-day risk. Institutions squaring positions. Avoid late entries.`);
  }
  if ((ind.rsi || 50) > 75 && (s.changePct || 0) > 3) {
    softWarns.push(`⚠️ RSI ${ind.rsi} overbought + up ${s.changePct}% today — do NOT chase. Wait for pullback.`);
  }
  if ((s.changePct || 0) >= 3) {
    softWarns.push(`⚠️ Stock already up ${s.changePct}% today. If entering, wait for 0.5–1% dip from current price.`);
  }

  const clear = blockers.filter(b => b.startsWith('❌') || b.startsWith('🔴')).length === 0;
  return { clear, blockers, softWarns, isThursday: dow === 4 };
}
