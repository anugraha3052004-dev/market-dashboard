...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        """Handle POST requests — currently only /ai-chat"""
        p    = urlparse(self.path)
        path = p.path
        if path == "/ai-chat":
            # Route to same handler as GET — it reads body itself
            self.do_GET_inner(path, {})
        else:
            self.send_json({"error":"POST not supported for this endpoint"},405)

    def do_GET(self):
        p    = urlparse(self.path)
        path = p.path
        flat = {k:v[0] for k,v in parse_qs(p.query).items()}
        self.do_GET_inner(path, flat)

    def do_GET_inner(self, path, flat):
        ...
        if path == "/etfs":
            # Correct NSE ETF symbols for Yahoo Finance (some need alternate tickers)
            ETF_SYMBOL_MAP = {
                "GOLDBEES":   "GOLDBEES.NS",
                "SILVERBEES": "SILVERBEES.NS",
                "COPPERBEES": "COPPERBEES.NS",
                "LIQUIDBEES": "LIQUIDBEES.NS",
                "NIFTYBEES":  "NIFTYBEES.NS",
                "JUNIORBEES": "JUNIORBEES.NS",
            }
            etf_syms = list(ETF_SYMBOL_MAP.keys())
            results = fetch_yahoo_multi(etf_syms, "10d")
            # For any that failed, try alternate symbols used by Yahoo for Indian ETFs
            ALTERNATE_SYMS = {
                "GOLDBEES":   "GOLDBEES",
                "COPPERBEES": "COPPERBEES",
            }
            failed = [s for s in etf_syms if results.get(s, {}).get("status") != "ok"]
            for sym in failed:
                # Mark as unavailable with placeholder
                results[sym] = {
                    "symbol": sym,
                    "name": {"GOLDBEES":"Nippon Gold ETF","COPPERBEES":"Nippon Copper ETF","SILVERBEES":"Nippon Silver ETF"}.get(sym, sym),
                    "price": 0, "change": 0, "changePct": 0,
                    "status": "unavailable",
                    "error": "Data temporarily unavailable — Yahoo Finance does not carry this ETF"
                }
            self.send_json(results); return
        ...
        if path == "/scan-fast":
            ...
            # If no BUY signals found, include top WATCH picks so the scanner is never empty
            picks_final = buys[:10]
            if len(picks_final) < 3 and watch:
                # Pad with top WATCH picks (mark them clearly)
                for w in watch[:max(0, 5 - len(picks_final))]:
                    w_copy = dict(w)
                    w_copy["signal"] = "WATCH"
                    picks_final.append(w_copy)

            # Add market context
            mkt_cached = cache_get("market-data")
            mkt_score = 3.0
            if mkt_cached:
                vix   = float((mkt_cached.get('vix') or {}).get('price') or 15)
                crude = float((mkt_cached.get('crude') or {}).get('price') or 78)
                nchg  = float((mkt_cached.get('nifty') or {}).get('changePct') or 0)
                s5chg = float((mkt_cached.get('sp500') or {}).get('changePct') or 0)
                mkt_score = max(0, min(5,
                    (2 if nchg >= 1 else 1 if nchg >= 0.3 else 0 if nchg > -0.3 else -1 if nchg >= -1 else -2) +
            ...
            mkt_verdict = "BULLISH" if mkt_score >= 4 else "CAUTION" if mkt_score >= 3 else "BEARISH" if mkt_score >= 2 else "UNSAFE"
        ...
        # ── AI CHAT — contextual follow-up about a stock ─────────────────
        if path == "/ai-chat":
            # Read POST body
            ...
            if not question:
                self.send_json({"answer": "Please ask a question."})
                return

            # Build a smart rule-based response (no external API needed)
            q = question.lower()
            answer = ""

            # Parse numbers from context for dynamic answers
            import re as _re
            ...
            sym_m   = _re.search(r'Stock:\s*([A-Z &]+)\s*\(([A-Z]+)\)', context)
            sym     = sym_m.group(2) if sym_m else 'the stock'
            name    = sym_m.group(1).strip() if sym_m else sym
            ...
            rsi_val = float(rsi_m.group(1)) if rsi_m else 50

            if 'No stock analysed' in context:
                answer = "⚠ Please run a Deep Analysis on a stock first, then ask your question here."

            elif any(w in q for w in ['shares','quantity','how many','qty','units','how much to buy']):
                risk_per = max(price - sl, 1) if price and sl else price * 0.025
                answer = (f"📊 Position Sizing for {sym}\n\n"
                         f"• Recommended: {int(qty)} shares @ ₹{price:.2f}\n"
                         f"• Capital deployed: ₹{invest:,.0f}\n"
                         f"• Risk per share: ₹{risk_per:.2f} (SL at ₹{sl:.0f})\n"
                         f"• Max loss at SL: ₹{max_loss:,.0f}\n\n"
                         f"This uses the 2% capital risk rule — never risk more than 2% of your total capital on a single trade. "
                         f"Scale up or down proportionally for your actual budget.")

            elif any(w in q for w in ['wait','when','entry','timing','buy now','enter now','right time']):
                if 'BUY' in signal:
                    answer = (f"⏰ Entry Timing for {sym}\n\n"
                             f"Signal: {signal} ({conf}% confidence) — Setup looks ready.\n\n"
                             f"• Best entry window: 3:00–3:20 PM IST (BTST buy)\n"
                             f"• Entry price: ₹{price:.2f} to ₹{price*1.005:.2f}\n"
                             f"• Do not chase if price is already up >1% from current\n"
                             f"• Market context: {mkt}\n\n"
                             f"Exit next morning 9:15–10:30 AM for BTST. Target ₹{t1:.0f} (T1) first.")
                else:
                    answer = (f"⏰ Entry Timing for {sym}\n\n"
                             f"Signal: {signal} ({conf}% confidence) — Not ideal to buy yet.\n\n"
                             f"• Wait for RSI to enter 45–65 range (currently {rsi_val:.0f})\n"
                             f"• Look for price to hold above ₹{sl:.0f} as support\n"
                             f"• Ideal entry on a pullback to support with volume confirmation\n"
                             f"• Market context: {mkt} — factor this into decision")

            elif any(w in q for w in ['stop loss','stop-loss','sl','stoploss','where.*stop','protect']):
                answer = (f"🛡 Stop Loss for {sym}\n\n"
                         f"• Hard SL: ₹{sl:.0f} (placed as a bracket/cover order)\n"
                         f"• That is {(price-sl)/price*100:.1f}% below your entry of ₹{price:.2f}\n"
                         f"• Risk per share: ₹{price-sl:.2f}\n"
                         f"• Max loss on {int(qty)} shares: ₹{max_loss:,.0f}\n\n"
                         f"Trail your SL:\n"
                         f"  → Once T1 (₹{t1:.0f}) hits — move SL to entry (₹{price:.2f})\n"
                         f"  → Once T2 (₹{t2:.0f}) hits — move SL to T1 (₹{t1:.0f})\n"
                         f"Always use a hard stop, not a mental stop.")

            elif any(w in q for w in ['target','profit','exit','t1','t2','t3','where.*sell','book']):
                answer = (f"🎯 Profit Targets for {sym}\n\n"
                         f"• T1: ₹{t1:.0f} (+1.5%) — Book 50% of position here\n"
                         f"• T2: ₹{t2:.0f} (+2.5%) — Book another 40% here\n"
                         f"• T3: ₹{_num('T3', t2*1.015):.0f} (+4%) — Trail stop for remaining 10%\n\n"
                         f"Expected move: {move} in {days}\n"
                         f"R:R Ratio: 1:{rr:.1f}\n\n"
                         f"For BTST: exit at T1 next morning if opening gap-up. "
                         f"If market opens flat, wait till 10:30 AM and exit regardless.")

            elif any(w in q for w in ['invalid','wrong','fail','break','stop working','exit.*setup','setup.*break']):
                answer = (f"⚠ Setup Invalidation for {sym}\n\n"
                         f"Exit immediately if:\n"
                         f"• Price closes BELOW ₹{sl:.0f} (hard stop)\n"
                         f"• Nifty falls >1.5% intraday on heavy volume\n"
                         f"• VIX spikes above 20 (fear index)\n"
                         f"• Any high-impact negative news (earnings miss, penalty, SEBI action)\n"
                         f"• Volume dries up with no follow-through after entry\n\n"
                         f"Rule: Once SL is hit, DO NOT average down. Exit clean and re-evaluate.")

            elif any(w in q for w in ['btst','tomorrow','next day','overnight','swing','hold overnight']):
                answer = (f"🌙 BTST Trade Plan for {sym}\n\n"
                         f"Signal: {signal} @ ₹{price:.2f}\n\n"
                         f"TODAY (Entry):\n"
                         f"• Buy {int(qty)} shares between 3:00–3:20 PM IST\n"
                         f"• Entry price: ₹{price:.2f} | Stop loss: ₹{sl:.0f}\n\n"
                         f"TOMORROW (Exit):\n"
                         f"• Exit window: 9:15–10:30 AM\n"
                         f"• Target: ₹{t1:.0f} (T1) → ₹{t2:.0f} (T2)\n"
                         f"• Exit regardless of P&L by 10:30 AM — do not hold for swing\n\n"
                         f"Setup: {setup}\n"
                         f"News sentiment: {sent}")

            elif any(w in q for w in ['confidence','sure','probability','reliable','accuracy','strong']):
                bars = '█' * (conf//10) + '░' * (10 - conf//10)
                answer = (f"📊 Confidence Score for {sym}: {conf}%\n\n"
                         f"[{bars}] {conf}%\n\n"
                         f"Based on 5-layer analysis:\n"
                         f"• Market: {mkt}\n"
                         f"• Technical setup: {setup}\n"
                         f"• RSI: {rsi_val:.0f} ({'Oversold — good entry zone' if rsi_val < 45 else 'Neutral' if rsi_val < 65 else 'Overbought — caution'})\n"
                         f"• News: {sent}\n\n"
                         f"{'✅ High confidence — multiple layers aligned. Normal position size.' if conf >= 70 else '🟡 Medium confidence — reduce position size by 50%.' if conf >= 55 else '⚠ Low confidence — paper trade only or skip.'}")

            elif any(w in q for w in ['risk','reward','r:r','risk reward','rr ratio']):
                answer = (f"⚖ Risk/Reward Analysis for {sym}\n\n"
                         f"• Entry: ₹{price:.2f}\n"
                         f"• Stop Loss: ₹{sl:.0f} → Risk = ₹{price-sl:.2f}/share\n"
                         f"• Target (T2): ₹{t2:.0f} → Reward = ₹{t2-price:.2f}/share\n"
                         f"• R:R Ratio: 1:{rr:.1f}\n\n"
                         f"{'✅ Good R:R (>1:2) — worth the trade.' if rr >= 2 else '🟡 Acceptable R:R — manage position size carefully.' if rr >= 1.5 else '⚠ Poor R:R (<1:1.5) — risk not justified. Consider skipping.'}\n\n"
                         f"Max loss: ₹{max_loss:,.0f} | Max gain at T2: ₹{t2*qty - price*qty:,.0f}")

            elif any(w in q for w in ['news','catalyst','event','announcement','reason']):
                answer = (f"📰 News & Catalyst for {sym}\n\n"
                         f"Sentiment: {sent}\n"
                         f"Market context: {mkt}\n\n"
                         f"Key things to check before entry:\n"
                         f"• NSE announcements page for {sym}\n"
                         f"• Any upcoming results/board meetings\n"
                         f"• Sector news (budget, policy, commodity prices)\n\n"
                         f"Rule: If you see HIGH-IMPACT negative news after analysis, skip the trade even if technicals look good. News trumps technicals for BTST.")

            else:
                # Generic summary
                answer = (f"🤖 STOCKSENSE Summary for {name} ({sym})\n\n"
                         f"Signal: {signal} ({conf}% confidence)\n"
                         f"Price: ₹{price:.2f} | Setup: {setup}\n\n"
                         f"Trade Plan:\n"
                         f"• Entry: ₹{price:.2f} ({int(qty)} shares = ₹{invest:,.0f})\n"
                         f"• SL: ₹{sl:.0f} | T1: ₹{t1:.0f} | T2: ₹{t2:.0f}\n"
                         f"• R:R = 1:{rr:.1f} | Expected: {move} in {days}\n\n"
                         f"Ask me: 'how many shares?', 'when to buy?', 'what's the SL?', "
                         f"'BTST plan', 'what invalidates this?', or 'confidence level?'")

            self.send_json({"answer": answer}); return
