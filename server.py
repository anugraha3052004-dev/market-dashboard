...
def score_stock_python(d):
    ...
    if not mh:
        # Pre-fetch market data inline so scoring is never wrong due to empty cache
        try:
            _market_syms = {"^INDIAVIX":"vix","^NSEI":"nifty","CL=F":"crude","^GSPC":"sp500"}
            _out = {}
            def _fetch_mkt(yf_sym, key):
                for base in YF_BASES:
                    try:
                        url = f"{base}/{yf_sym}?interval=1d&range=5d"
                        req = Request(url, headers=get_yf_headers())
                        ...
                        meta = _d["chart"]["result"][0]["meta"]
                        price = float(meta.get("regularMarketPrice") or meta.get("previousClose") or 0)
                        prev  = float(meta.get("previousClose") or price)
                        _out[key] = {"price":round(price,2),"changePct":round((price-prev)/prev*100 if prev else 0,2),"status":"ok"}
                        return
                    except: pass
                _out[key] = {"status":"error"}
            _threads = [threading.Thread(target=_fetch_mkt, args=(s,k),daemon=True) for s,k in _market_syms.items()]
            for t in _threads: t.start()
            for t in _threads: t.join(timeout=8)
            if _out:
                cache_set("market-data", _out)
                mh = _out
        except: pass
    if mh:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/etfs":
            ...
            # ── Gold/Silver/Copper ETFs via gold-api.com + USD/INR conversion ──
            # These NSE ETFs track international metal prices so we derive them
            try:
                usdInr_d = fetch_td("/quote", {"symbol":"USD/INR","dp":"2"})
                usdInr = float(usdInr_d.get("close") or usdInr_d.get("price") or 84)
            except:
            ...
            metal_etfs = {
                "GOLDBEES":   {"metal":"XAU","name":"Nippon Gold ETF","unit_g":1,"factor":lambda p,fx: round((p/31.1035)*fx*1, 2)},
                "SILVERBEES": {"metal":"XAG","name":"Nippon Silver ETF","unit_g":1,"factor":lambda p,fx: round((p/31.1035)*fx*1000/100, 2)},
                "COPPERBEES": {"metal":"XCU","name":"Nippon Copper ETF","unit_g":1,"factor":lambda p,fx: round((p/0.453592)*fx/100, 2)},
            }
            for sym, cfg in metal_etfs.items():
                try:
                    url = f"https://api.gold-api.com/price/{cfg['metal']}"
                    req = Request(url, headers={"User-Agent":"MarketDashboard/1.0","Accept":"application/json"})
                    with urlopen(req, timeout=8) as r:
                        gd = json.loads(r.read())
                    spot = float(gd.get("price",0))
                    prev_spot = float(gd.get("prev_close_price", spot))
                    if spot > 0:
                        # NAV approximation from metal price
                        nav = cfg["factor"](spot, usdInr)
                        nav_prev = cfg["factor"](prev_spot, usdInr)
                        chg = round(nav - nav_prev, 2)
                        pct = round((chg/nav_prev*100) if nav_prev else 0, 2)
                        results[sym] = {
                            "symbol":sym, "name":cfg["name"],
                            "price":nav, "change":chg, "changePct":pct,
                            "prevClose":nav_prev, "volume":0,
                            "status":"ok",
                            "note":"NAV derived from international metal prices",
                            "indicators":{"rsi":None,"sma20":None,"sma50":None,"ema9":None,
                                         "macd":None,"macdSignal":None,"macdHist":None,
                                         "support":round(nav*0.97,2),"resistance":round(nav*1.03,2),
                                         "volumeSignal":"normal"},
                            "recommendation":"HOLD","sentiment":"Neutral","confidence":50,
                            "reasons":["Tracks international metal price (gold-api.com)"]
                        }
                    else:
                        raise ValueError("Zero price")
                except Exception as e:
                    print(f"  [ETF] {sym} gold-api error: {e}")
                    results[sym] = {"symbol":sym,"name":cfg["name"],"price":0,"change":0,"changePct":0,
                                    "status":"unavailable","reasons":["Metal price fetch failed"]}

            # ── Index/Liquid ETFs via Yahoo Finance ──
            idx_etfs = ["NIFTYBEES","JUNIORBEES","LIQUIDBEES"]
            idx_results = fetch_yahoo_multi(idx_etfs, "10d")
            for sym in idx_etfs:
                r = idx_results.get(sym, {})
                if r.get("status") == "ok" and r.get("price"):
                    results[sym] = r
                else:
                    names = {"NIFTYBEES":"Nippon Nifty 50 ETF","JUNIORBEES":"Nippon Junior BeES","LIQUIDBEES":"Nippon Liquid ETF"}
                    results[sym] = {"symbol":sym,"name":names.get(sym,sym),"price":0,"change":0,"changePct":0,
                                    "status":"unavailable","reasons":["Data not available from Yahoo Finance"]}
        ...
        if path == "/scan-fast":
            ...
            # Pre-fetch market data if not cached — ensures accurate Layer 1 scoring
            if not cache_get("market-data"):
                print("  [SCAN-FAST] Market data not cached — fetching before scan…")
                _mkt_syms = {"^INDIAVIX":"vix","^NSEI":"nifty","CL=F":"crude","^GSPC":"sp500"}
                _mkt_out  = {}
                def _pre_fetch_mkt(yf_sym, key):
                    for base in YF_BASES:
                        try:
                            url = f"{base}/{yf_sym}?interval=1d&range=5d"
                            req = Request(url, headers=get_yf_headers())
                            ...
                            price = float(meta.get("regularMarketPrice") or meta.get("previousClose") or 0)
                            prev  = float(meta.get("previousClose") or price)
                            _mkt_out[key] = {"price":round(price,2),"changePct":round((price-prev)/prev*100 if prev else 0,2),"status":"ok"}
                            return
                        except: pass
                    _mkt_out[key] = {"status":"error","price":0,"changePct":0}
                _mkt_threads = [threading.Thread(target=_pre_fetch_mkt, args=(s,k),daemon=True) for s,k in _mkt_syms.items()]
                for t in _mkt_threads: t.start()
                for t in _mkt_threads: t.join(timeout=10)
                if _mkt_out: cache_set("market-data", _mkt_out)
                print(f"  [SCAN-FAST] Market data pre-fetched: {_mkt_out}")
        ...
        # ══════════════════════════════════════════════════════════════════
        # 3-STAGE SMART PIPELINE — /smart-pipeline
        # Stage 1: Bulk filter via Yahoo Finance (free, no limit)
        # Stage 2: Multi-layer scoring (trend, volume, RSI, MACD, news)
        # Stage 3: Real-time validation via Twelve Data (top picks only)
        # Returns: Top 3 final picks + Best Pick of Day
        # ══════════════════════════════════════════════════════════════════
        if path == "/smart-pipeline":
            force     = flat.get("force","").lower() == "true"
            mode      = flat.get("mode","nse30")  # nse30 | nse50 | all
            watchlist = [s.strip().upper() for s in flat.get("watchlist","").split(",") if s.strip()]

            # ── Cache check (30 min) ──────────────────────────────────────
            cache_key = f"smart-pipeline-{mode}"
            if not force:
                cached = cache_get(cache_key)
                if cached:
            ...
            pipeline_log = []  # audit trail for transparency

            # ── UNIVERSE DEFINITION ───────────────────────────────────────
            # 50 most liquid NSE F&O stocks — broadest free Yahoo coverage
            UNIVERSE_50 = [
                ...
                "CIPLA","TITAN","NESTLEIND","TECHM","HINDALCO","BAJAJFINSV",
                "ULTRACEMCO","ASIANPAINT","BRITANNIA","DIVISLAB","GRASIM",
                "HEROMOTOCO","INDUSINDBK","M&M","BPCL","COALINDIA","EICHERMOT",
                "APOLLOHOSP","TATACONSUM","LTIM","PERSISTENT","MUTHOOTFIN",
                "PIDILITIND","SBILIFE","HDFCLIFE"
            ]
            UNIVERSE_30 = UNIVERSE_50[:30]
            ETF_UNIVERSE = ["NIFTYBEES","JUNIORBEES","GOLDBEES","SILVERBEES"]

            if mode == "all":
                universe = UNIVERSE_50 + ETF_UNIVERSE
            elif mode == "nse50":
                universe = UNIVERSE_50
            elif mode == "watchlist" and watchlist:
                universe = watchlist[:30]
            else:  # nse30 default
                universe = UNIVERSE_30

            pipeline_log.append(f"Stage 0: Universe = {len(universe)} stocks (mode={mode})")

            # ══════════════════════════════════════════════════════════════
            # STAGE 1 — BULK FILTER via Yahoo Finance (FREE, no API limit)
            # Fetch 30d history for entire universe in parallel batches
            # Filter criteria:
            #   RSI: 35–72 (not oversold enough to be broken, not overbought)
            #   Volume: last 3-day avg >= 1.0x 10-day avg (participation)
            #   Trend: 7d change > -3% (not in freefall)
            #   Price: between support*1.01 and resistance*0.99 OR at breakout
            # ══════════════════════════════════════════════════════════════
            print(f"\n  [PIPELINE] Stage 1 — Fetching {len(universe)} stocks from Yahoo…")
            all_data = {}
            BATCH = 8
            for i in range(0, len(universe), BATCH):
                batch = universe[i:i+BATCH]
                result = fetch_yahoo_multi(batch, "30d")
            ...
            for sym, d in all_data.items():
                if d.get("status") != "ok" or not d.get("price"):
                    stage1_rejected.append({"sym":sym,"reason":"fetch_failed"})
                    continue
                ...
                # ── Volume filter: recent 3-day avg vs 10-day avg ──
                vols = [h.get("volume",0) for h in hist[-10:] if h.get("volume",0) > 0]
                vol_10avg = sum(vols) / len(vols) if vols else 0
                vol_3avg  = sum(vols[-3:]) / 3 if len(vols) >= 3 else vol_10avg
                vol_ratio = vol_3avg / vol_10avg if vol_10avg > 0 else 1.0

                # ── 7-day trend ──
                trend_7d = 0
                if len(hist) >= 8:
                    try: trend_7d = (price - hist[-8]["close"]) / hist[-8]["close"] * 100
                    except: pass

                # ── 30-day trend ──
                trend_30d = 0
                if len(hist) >= 20:
                    try: trend_30d = (price - hist[-20]["close"]) / hist[-20]["close"] * 100
                    except: pass

                # ── STAGE 1 FILTER RULES ──
                reject_reason = None
                if rsi < 30:           reject_reason = f"RSI {rsi:.0f} — deeply oversold/broken"
                elif rsi > 78:         reject_reason = f"RSI {rsi:.0f} — severely overbought"
                elif trend_7d < -4:    reject_reason = f"7d trend {trend_7d:.1f}% — strong downtrend"
                elif vol_ratio < 0.4:  reject_reason = f"Volume dead — ratio {vol_ratio:.2f}"

                if reject_reason:
                    stage1_rejected.append({"sym":sym,"reason":reject_reason})
                    continue

                # Passed Stage 1 — attach extra computed fields
                d["_trend_7d"]   = round(trend_7d, 2)
                d["_trend_30d"]  = round(trend_30d, 2)
                d["_vol_ratio"]  = round(vol_ratio, 2)
                d["_vol_10avg"]  = int(vol_10avg)
                stage1_passed.append(d)

            pipeline_log.append(f"Stage 1: {len(universe)} → {len(stage1_passed)} passed filter "
                                 f"({len(stage1_rejected)} rejected)")
            print(f"  [PIPELINE] Stage 1 done: {len(stage1_passed)}/{len(universe)} passed")

            # ══════════════════════════════════════════════════════════════
            # STAGE 2 — MULTI-LAYER SCORING (no extra API calls)
            # Score each stock across 5 dimensions (total 100 points):
            #   Trend Strength   25pts
            #   Volume Confirm   20pts
            #   RSI Position     15pts
            #   MACD Signal      20pts
            #   News Sentiment   20pts
            # Select top 5–8 for Stage 3
            # ══════════════════════════════════════════════════════════════
            def stage2_score(d):
                ind     = d.get("indicators", {})
                price   = d.get("price", 0)
                rsi     = float(ind.get("rsi") or 50)
                macd_h  = float(ind.get("macdHist") or 0)
                macd_v  = float(ind.get("macd") or 0)
                vol_sig = ind.get("volumeSignal","normal")
                sma20   = float(ind.get("sma20") or 0)
                sma50   = float(ind.get("sma50") or 0)
                support = float(ind.get("support") or 0)
                ...
                sent    = d.get("sentiment","Neutral")
                rec     = d.get("recommendation","HOLD")
                t7      = d.get("_trend_7d", 0)
                t30     = d.get("_trend_30d", 0)
                vol_r   = d.get("_vol_ratio", 1.0)
                scores  = {}

                # ── 1. TREND STRENGTH (25 pts) ──────────────────────────
                ts = 0
                # 7-day trend
                if t7 >= 3:    ts += 10
                elif t7 >= 1:  ts += 7
                ...
                # SMA alignment (price above both = strong uptrend)
                if sma20 and sma50 and price > sma20 > sma50: ts += 7
                elif sma20 and price > sma20:                  ts += 4
                elif sma50 and price > sma50:                  ts += 2
                scores["trend"] = min(25, ts)

                # ── 2. VOLUME CONFIRMATION (20 pts) ─────────────────────
                vs = 0
                if vol_r >= 2.0:    vs = 20   # massive volume surge
                elif vol_r >= 1.5:  vs = 16   # strong spike
                elif vol_r >= 1.2:  vs = 12   # above average
                elif vol_r >= 1.0:  vs = 8    # normal
                elif vol_r >= 0.7:  vs = 4    # below average
                else:               vs = 1
                if vol_sig == "high": vs = min(20, vs + 3)  # bonus for server-side detection
                scores["volume"] = vs

                # ── 3. RSI POSITION (15 pts) ─────────────────────────────
                rs = 0
                # Optimal BTST zone: 50–65 (momentum without overbought)
                if 52 <= rsi <= 63:    rs = 15   # perfect zone
                elif 45 <= rsi <= 68:  rs = 11
                elif 38 <= rsi <= 72:  rs = 7
                elif 33 <= rsi <= 75:  rs = 3
                else:                   rs = 0
                scores["rsi"] = rs

                # ── 4. MACD SIGNAL (20 pts) ──────────────────────────────
                ms = 0
                # Histogram positive and growing = bullish momentum
                if macd_h > 0 and macd_v > 0:  ms = 20   # both positive
                elif macd_h > 0:                ms = 14   # histogram just turned positive
                elif macd_h > -0.5:             ms = 7    # near zero — watching
                else:                           ms = 0
                scores["macd"] = ms

                # ── 5. NEWS SENTIMENT (20 pts) ───────────────────────────
                ns = 0
                if sent == "Bullish":            ns = 20
                elif sent == "Neutral":          ns = 10
                elif sent == "Bearish":          ns = 2
                if rec == "BUY":   ns = min(20, ns + 4)
                if rec == "AVOID": ns = max(0,  ns - 6)
                # Price action bonus (momentum catalyst)
                if chg_pct > 2:    ns = min(20, ns + 4)
                elif chg_pct > 1:  ns = min(20, ns + 2)
                scores["news"] = ns

                total = sum(scores.values())

                # ── Setup type ───────────────────────────────────────────
                if resist and price >= resist * 0.998:
                    setup = "Breakout 🚀"
                elif resist and price >= resist * 0.97 and chg_pct > 0.5:
                    setup = "Near Breakout ⚡"
                elif support and price <= support * 1.025 and chg_pct >= 0:
                    setup = "Support Bounce 📈"
                elif 50 <= rsi <= 65 and macd_h > 0 and t7 > 0:
                    setup = "Momentum Continuation 🔥"
                elif t7 > 1 and vol_r >= 1.5:
                ...
                return {
                    ...
                    "vol_ratio": d.get("_vol_ratio"),
                    "trend_7d":  d.get("_trend_7d"),
                    "trend_30d": d.get("_trend_30d"),
                    "sentiment": sent,
                    "rec":       rec,
                    "_raw":      d,   # keep for Stage 3
                }

            stage2_results = []
            for d in stage1_passed:
                scored = stage2_score(d)
                if scored["total"] >= 30:   # minimum threshold to qualify
                    stage2_results.append(scored)

            stage2_results.sort(key=lambda x: -x["total"])
            top_candidates = stage2_results[:8]   # send top 8 to Stage 3

            pipeline_log.append(f"Stage 2: {len(stage1_passed)} → {len(stage2_results)} qualified "
                                 f"(score≥30), top {len(top_candidates)} sent to Stage 3")
            print(f"  [PIPELINE] Stage 2 done: {len(top_candidates)} candidates for Stage 3")

            # ══════════════════════════════════════════════════════════════
            # STAGE 3 — REAL-TIME VALIDATION via Twelve Data
            # ONLY for top 8 candidates — minimises API calls
            # Validates:
            #   - Latest price vs Yahoo price (confirm no big gap)
            #   - Confirms momentum is still active
            #   - Adjusts entry/SL/targets with real-time price
            # ══════════════════════════════════════════════════════════════
            td_calls_used = 0
            final_picks   = []
            ...
            for cand in top_candidates:
                ...
                # Try Twelve Data real-time price (if API key set)
                rt_price = yf_price   # default to Yahoo price
                rt_source = "yahoo"
                price_drift = 0.0

                if API_KEY and td_calls_used < 8:
                    try:
                        td_url = f"{TWELVE_DATA_BASE}/price?symbol={sym}:NSE&apikey={API_KEY}"
                        req = Request(td_url, headers={"User-Agent":"StockSense/1.0"})
                        with urlopen(req, timeout=8) as r:
                            td_data = json.loads(r.read())
                        if td_data.get("price"):
                            rt_price   = float(td_data["price"])
                            rt_source  = "twelvedata"
                            price_drift = round(abs(rt_price - yf_price) / yf_price * 100, 2)
                            td_calls_used += 1
                            print(f"  [PIPELINE] TD validation {sym}: ₹{rt_price} (drift {price_drift}%)")
                    except Exception as e:
                        print(f"  [PIPELINE] TD failed for {sym}: {e}")

                # ── Validation rules ──────────────────────────────────────
                # Reject if real-time price drifted too far from Yahoo (stale data)
                if price_drift > 3.0:
                    pipeline_log.append(f"Stage 3: {sym} REJECTED — price drift {price_drift}% too large")
                    continue

                # ── Trade levels using real-time price ────────────────────
                ind      = d_raw.get("indicators", {})
                support  = float(ind.get("support") or rt_price * 0.97)
                resist   = float(ind.get("resistance") or rt_price * 1.04)
                sma20    = float(ind.get("sma20") or 0)
                rsi      = cand["rsi"]
                score    = cand["total"]
                setup    = cand["setup"]
                ...
                # Stop loss: just below support or 2.5% below entry
                sl_support = support * 0.995 if support else 0
                sl_pct     = rt_price * 0.975
                sl         = round(max(sl_support, sl_pct), 1)

                # Targets: based on setup type
                if "Breakout" in setup:
                    t1 = round(rt_price * 1.018, 1)   # 1.8%
                    t2 = round(rt_price * 1.030, 1)   # 3.0%
                elif "Momentum" in setup or "Volume" in setup:
                    t1 = round(rt_price * 1.015, 1)   # 1.5%
                    t2 = round(rt_price * 1.025, 1)   # 2.5%
                elif "Bounce" in setup:
                    t1 = round(rt_price * 1.012, 1)   # 1.2%
                    t2 = round(rt_price * 1.020, 1)   # 2.0%
                else:
                    t1 = round(rt_price * 1.012, 1)
                    t2 = round(rt_price * 1.020, 1)

                risk   = round(((rt_price - sl) / rt_price) * 100, 1)
                reward = round(((t2 - rt_price) / rt_price) * 100, 1)
                rr     = round(reward / risk, 1) if risk > 0 else 0

                # Confidence: based on Stage 2 score + real-time validation
                conf = min(92, round((score / 100) * 100))
                if rt_source == "twelvedata":  conf = min(92, conf + 5)   # bonus for RT validation
                if price_drift < 0.5:          conf = min(92, conf + 3)   # price very fresh
                if rr >= 2.0:                  conf = min(92, conf + 3)   # good R:R

                # Risk level
                risk_level = "LOW" if (rr >= 2.0 and risk <= 2.5 and score >= 70) else \
                             "MEDIUM" if (rr >= 1.5 and risk <= 3.5) else "HIGH"

                # Expected move
                exp_move = f"+{t1-rt_price:.0f}–{t2-rt_price:.0f} (₹) / +{reward:.1f}% in 2–3 days"
                ...
                if sc["trend"] >= 18:
                    reasoning.append(f"Strong uptrend: +{t7:.1f}% (7d), +{t30:.1f}% (30d). Price above key MAs.")
                elif sc["trend"] >= 10:
                    reasoning.append(f"Positive trend: +{t7:.1f}% (7d). Momentum building.")
                else:
                    reasoning.append(f"Trend: {t7:.1f}% (7d) — cautious but qualified.")

                if vr >= 1.5:
                    reasoning.append(f"Volume surge: {vr:.1f}× average — institutional participation confirmed.")
                elif vr >= 1.2:
                    reasoning.append(f"Above-average volume ({vr:.1f}×) — buyers stepping in.")
                else:
                    reasoning.append(f"Volume normal ({vr:.1f}×) — watch for expansion.")

                if sc["rsi"] >= 11:
                    reasoning.append(f"RSI {rsi:.0f} — in optimal BTST zone (50–65). Not overbought.")
                else:
                    reasoning.append(f"RSI {rsi:.0f} — monitor for entry confirmation.")

                macd_h = cand["macd_hist"]
                if sc["macd"] >= 14:
                    reasoning.append(f"MACD histogram positive ({macd_h:+.2f}) — bullish momentum confirmed.")
                elif sc["macd"] >= 7:
                    reasoning.append(f"MACD near crossover ({macd_h:+.2f}) — watch for signal.")
                else:
                    reasoning.append(f"MACD negative — setup needs volume/price confirmation.")

                sent = cand["sentiment"]
                if sc["news"] >= 16:
                    reasoning.append(f"Sentiment: {sent}. News flow supportive for short-term move.")
                else:
                    reasoning.append(f"Sentiment: {sent}. No major negative news risk.")

                final_picks.append({
                    "rank":         len(final_picks) + 1,
                    "symbol":       sym,
                    "name":         cand["name"],
                    "price":        round(rt_price, 2),
                    "yf_price":     round(yf_price, 2),
                    "price_source": rt_source,
                    "price_drift":  price_drift,
                    "changePct":    cand["changePct"],
                    "setup":        setup,
                    "signal":       "BUY" if score >= 60 else "WEAK BUY",
                    "confidence":   conf,
                    "score":        score,
                    "scores":       sc,
                    "sl":           sl,
                ...
                if len(final_picks) >= 3:  # cap at top 3
                    break

            pipeline_log.append(f"Stage 3: {len(top_candidates)} validated → {len(final_picks)} final picks "
                                 f"({td_calls_used} Twelve Data calls used)")
            print(f"  [PIPELINE] Stage 3 done: {len(final_picks)} final picks, {td_calls_used} TD calls used")

            # ── BEST PICK OF THE DAY ──────────────────────────────────────
            best_pick = None
            if final_picks:
                best_pick = final_picks[0]
                bp = best_pick
                best_pick["best_pick_reason"] = (
                    f"{bp['name']} stands out because: "
                    f"score {bp['score']}/100 (#{1} overall), "
                    f"setup '{bp['setup']}', "
                    f"RSI {bp['rsi']:.0f} in optimal zone, "
                    f"volume {bp['vol_ratio']:.1f}× average, "
                    f"R:R = 1:{bp['rr']:.1f}. "
            ...
            resp = {
                ...
                "stage2_qualified": len(stage2_results),
                "stage3_final":   len(final_picks),
                "td_calls_used":  td_calls_used,
                "pipeline_log":   pipeline_log,
                "best_pick":      best_pick,
                "top3":           final_picks,
                "all_stage2":     [
                    {k:v for k,v in s.items() if k != "_raw"}
                    for s in stage2_results[:15]
                ],
                "mode": mode,
            }
